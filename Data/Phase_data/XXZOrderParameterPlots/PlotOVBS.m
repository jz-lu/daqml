...
for i = 1:length(Jratios)
    for j = 1:length(alphas)
        op(i,j) = getOVBS(Jratios(i),1,alphas(j),Lx);
    end
end
...
%imagesc(Jratios,alphas,op',[0 16.1])
axis xy 
hold on

f = 0.91; %percentile in the range of op vals I am selecting for contour line
...
VBS_contour_xvals = M(1,2: (npoint + 1)); % saving x values
VBS_contour_yvals = M(2,2: (npoint + 1)); % saving y values

folder = 'C:\Users\Kristina\Desktop\QML';
filename = [folder '\' 'VBS_contour.mat'];
save(filename, 'VBS_contour_xvals', 'VBS_contour_yvals');

op_filename = [folder '\' 'vbs_op.mat'];
op_prime = op';
save(op_filename, 'op_prime');

colorbar
title(sprintf('VBS order parameter %d-qubit mesh', Lx))
xlabel('J3/J6') 
ylabel('alpha') 
