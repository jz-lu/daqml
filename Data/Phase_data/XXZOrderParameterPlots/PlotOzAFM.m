...
for i = 1:length(Jratios)
    for j = 1:length(alphas)
        op(i,j) = getzAFM(Jratios(i),1,alphas(j),Lx);
    end
end
...
figure
%contourf(X,Y,op',20)
imagesc(Jratios,alphas,op',[0 6.5])
axis xy 
hold on

f = 0.82; %percentile in the range of op vals I am selecting for contour line
...
zafm_contour_xvals = M(1,2: (npoint + 1)); % saving x values
zafm_contour_yvals = M(2,2: (npoint + 1)); % saving y values

folder = 'C:\Users\Kristina\Desktop\QML';
filename = [folder '\' 'zafm_contour.mat'];
save(filename, 'zafm_contour_xvals', 'zafm_contour_yvals');

op_filename = [folder '\' 'zafm_op.mat'];
op_prime = op';
save(op_filename, 'op_prime');

colorbar
title(sprintf('zAFM order parameter %d-qubit mesh', Lx))
xlabel('J3/J6') 
ylabel('alpha') 
