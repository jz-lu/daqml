...
for i = 1:length(Jratios)
    for j = 1:length(alphas)
        op(i,j) = getqzAFM(Jratios(i),1,alphas(j),Lx);
    end
end
...
f = 0.97; %percentile in the range of op vals I am selecting for contour line
...
qzafm_contour_xvals = M(1,2: (npoint + 1)); % saving x values
qzafm_contour_yvals = M(2,2: (npoint + 1)); % saving y values

folder = 'C:\Users\Kristina\Desktop\QML';
filename = [folder '\' 'qzafm_contour.mat'];
save(filename, 'qzafm_contour_xvals', 'qzafm_contour_yvals');

op_filename = [folder '\' 'qzafm_op.mat'];
op_prime = op';
save(op_filename, 'op_prime');

colorbar
title(sprintf('qzAFM order parameter %d-qubit mesh', Lx))
xlabel('J3/J6') 
ylabel('alpha') 