...
# use little endian for julia can match this with the cnot gate from yao
function noisy_cnot_approx_gate(theta::Float64)
    ...
    little_endian = [
        [1, 0, 0, 0],
        [0, 0.5 * (1 + exp_term), 0, 0.5 - 0.5 * exp_term],
        [0, 0, 1, 0],
        [0, 0.5 * (1 - exp_term), 0, 0.5 + 0.5 * exp_term],
    ]
    le = vecvec_to_matrix(little_endian)
    return matblock(le, tag=string("crot", theta))
end
...
# digital parameterized cnot gate
function ent_pcx(nbit::Int64, theta::Float64)
    return (nbit%2 == 0) ? 
    chain(chain(nbit,put((i, i+1)=>noisy_cnot_approx_gate(theta)) for i in 1:2:nbit-1),
        chain(nbit,put((i, i+1)=>noisy_cnot_approx_gate(theta)) for i in 2:2:nbit-2)) : 
    chain(chain(nbit,put((i, i+1)=>noisy_cnot_approx_gate(theta)) for i in 1:2:nbit-2),
        chain(nbit,put((i, i+1)=>noisy_cnot_approx_gate(theta)) for i in 2:2:nbit-1))
end

# digital parameterized cnot gate with noise (add noise to angle of rotation)
function ent_pcx_noisy(nbit::Int64, theta::Float64, sd::Float64)
    param_noise = Normal(0, sd)

    return (nbit%2 == 0) ? 
    chain(chain(nbit,put((i, i+1)=>noisy_cnot_approx_gate(theta + rand(param_noise, 1)[1])) for i in 1:2:nbit-1),
        chain(nbit,put((i, i+1)=>noisy_cnot_approx_gate(theta + rand(param_noise, 1)[1])) for i in 2:2:nbit-2)) : 
    chain(chain(nbit,put((i, i+1)=>noisy_cnot_approx_gate(theta +rand(param_noise, 1)[1])) for i in 1:2:nbit-2),
        chain(nbit,put((i, i+1)=>noisy_cnot_approx_gate(theta + rand(param_noise, 1)[1])) for i in 2:2:nbit-1))
end
...
function ryd_h(nsites::Int64, Rb_over_a::Float64, Δ_over_Ω::Float64, noise::Bool=false)
    ...
    if !noise  
        ...
        x_coords = LinRange(0, nsites * a - a, nsites)
